...
def calculate_vhf_for_lag_time(traj, lag_time, bins):
    ...
    return hist, hist_x, hist_y, hist_z
...
def calc_vhf(traj, atom):
    ...
    with ProcessPoolExecutor(max_workers=128) as executor:
        results = list(tqdm(executor.map(calculate_vhf_for_lag_time, [traj] * n_lag_times, lag_times, [bins] * n_lag_times), total=n_lag_times, disable=False))

    for i, (hist, hist_x, hist_y, hist_z) in enumerate(results):
        dis[i] = np.mean(hist)
