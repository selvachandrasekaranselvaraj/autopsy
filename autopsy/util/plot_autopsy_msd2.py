    ...
    for msd_type in ['self', 'distinct', 'total']:        
        for specie in grep_autopsy_species(folders):
            ...
            if labels:
                ...
                plot_msd(msds, ds, times, labels, output_name, skip_indices)
    return
...
def plot_msd(msds, diff_co, times, labels, output_name, skip_indices):  
    ...
    for msd, d, c, label, time in zip(msds, diff_co, colors, labels, times):
        ...
        poly_order = 1
        n_smooth = 2
        slop_interval = int(len(msd)*0.05)
        legend_length += len(label)
        # Apply Savitzky-Golay filter to smooth the noisy data
        smoothed_msd = smooth_out(msd, n_smooth, window_size, poly_order)
        time = np.array(time)   
        ...
        fig.add_trace(go.Scatter(x=time[skip_indices:-skip_indices], 
                                 y=msd[skip_indices:-skip_indices], 
        ...
        #################################
        fig.add_trace(go.Scatter(x=time[skip_indices:-skip_indices], 
                                 y=smoothed_msd[skip_indices:-skip_indices], 
                                 line=dict(color=c),
                                 name = label, 
                                 showlegend=True), 
                      row=1, 
        ...
        if isinstance(d, np.ndarray):
            ...
            fig.add_trace(go.Scatter(x=time[skip_indices+interval:-skip_indices], 
                                     y=diff_co[skip_indices:-skip_indices], 
                                     line=dict(color=c),
                                     name = label, 
                                     showlegend=False), 
                          row=2, 
        ...
        fig.add_trace(go.Scatter(x=np.log10(time[skip_indices:-skip_indices]), 
                                 y=smoothed_log_msd[skip_indices:-skip_indices], 
                                 line=dict(color=c), 
                                 showlegend=False), 
                      row=1, col=2)
